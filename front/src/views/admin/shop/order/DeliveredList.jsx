...
function DeliveredList() {
    ...
    const loadOrderList = async () => {
        try {
            const res = await axiosInstance.get(`/admin/shop/order/list/delivered`);
    ...
    const submitCompleted = async () => {
        try {
            ...
            const body = { order_codes: checkedItems, status: 'COMPLETED' };
            const response = await axiosInstance.put(`/admin/shop/order/status`, body);
            if (response.data.success) {
                loadOrderList();
                setCheckedItems([]);
                toaster.create({ title: '구매확정 처리되었습니다.', type: 'success' });
            }
        } catch (e) {
            console.error(e);
    ...
    const submitShipping = async () => {
        try {
            ...
            const body = { order_codes: checkedItems, status: 'SHIPPING' };
            const response = await axiosInstance.put(`/admin/shop/order/status`, body);
            if (response.data.success) {
                loadOrderList();
                setCheckedItems([]);
                toaster.create({ title: '배송중으로 이동되었습니다.', type: 'success' });
            }
        } catch (e) {
            console.error(e);
            toaster.create({ title: '오류가 발생했습니다.', type: 'error' });
    ...
    return (
        <Stack p="30px" px="layoutX" gap="6">
            ...
            <HStack>
                <Button size="xs" onClick={submitCompleted}>선택 상품 구매확정 처리</Button>
                <Button size="xs" variant="outline" onClick={submitShipping}>선택 상품 배송중으로 이동</Button>
            </HStack>
            ...
            <Table.Root>
                ...
                <Table.Body>
                    {orderList.map((order) => {
                        const firstItem = order.product_order_items[0];
                        const isAllSamePost = order.product_order_items.length > 0 && order.product_order_items.every(
                            (item) => item.post_company === firstItem.post_company && item.post_number === firstItem.post_number
                        );
                        return (
                            <Table.Row key={order.order_code}>
                                ...
                                <Table.Cell pr="0">
                                    <Stack separator={<StackSeparator />} gap="2" >
                                        {order.product_order_items.map((orderItem) => (
                                            <HStack key={orderItem.order_item_code} gap="5" pl="2" minH="12">
                                ...
                                <Table.Cell px="0">
                                    <Stack separator={<StackSeparator />} gap="2">
                                        {order.product_order_items.map((orderItem) => (
                                            <HStack key={orderItem.order_item_code} justify="center" minH="12">
                                                <Text textAlign="center">{orderItem.quantity}</Text>
                                            </HStack>
                                        ))}
                                    </Stack>
                                </Table.Cell>
                                <Table.Cell textAlign="center" px={isAllSamePost ? "3" : "0"}>
                                    <Stack separator={<StackSeparator />} gap="2">
                                        {isAllSamePost ? (
                                            <Stack minH="12" gap="1" justify="center">
                                                <HStack justify="center" fontSize="xs">
                                                    <Text>{firstItem.post_company}</Text>
                                                    <Text>{firstItem.post_number}</Text>
                                                </HStack>
                                                <Button size="2xs" fontSize="xs">배송 추적</Button>
                                            </Stack>
                                        ) : (
                                            order.product_order_items.map((orderItem) => (
                                                <Stack key={orderItem.order_item_code} minH="12" gap="1">
                                                    <HStack justify="center" fontSize="xs">
                                                        <Text>{orderItem.post_company}</Text>
                                                        <Text>{orderItem.post_number}</Text>
                                                    </HStack>
                                                    <Button size="2xs" fontSize="xs">배송 추적</Button>
                                                </Stack>
                                            ))
